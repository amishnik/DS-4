...
SELECT agegroup AS "Возрастная группа", SUM(sum) as total, COUNT(sum) AS "Количество транзакций", SUM(sum) / COUNT(sum) AS sum_tr, COUNT(DISTINCT client_id) AS client_num,
--среднее число транзакций на клиента
CASE WHEN COUNT(sum) > 0 THEN (COUNT(sum)::decimal/ COUNT(DISTINCT Client_id)::decimal) ELSE 0 END AS avg_tr,
...
--добавляю столюец для учета успешной коммуникации
...
UPDATE activities a SET StatusName = Statecode FROM activitystatus s WHERE a.Statuscode=s.Statuscode;
	...
	FROM ttarget_clients ttc JOIN activities a ON ttc.clientid=a.client_id 
...
SELECT c3.clientid, COUNT(activityid), (SELECT AVG(total_comm_num) FROM target_comm tc JOIN clients c ON tc.clientid=c.clientid WHERE agegroup ='от 35 до 45') AS avg 
	, SUM(CASE WHEN StatusName='1' THEN 1 ELSE 0 END) AS success_comm_num
	--смотрим соотношение числа успешных коммуникаций к неуспешным
	, (SUM(CASE WHEN StatusName='1' THEN 1 ELSE 0 END)::decimal / COUNT(activityid)::decimal)*100 AS comm_diff 
